...
const generateProceduralNpc = (moral) => {
  ...
  // 1% Emperor chance (only triggered at good or medium spirit: moral >= 40)
  if (m >= 40 && Math.random() < 0.01) {
    const isSviatozar = Math.random() < 0.5;
    if (isSviatozar) {
      return {
      ...
      return {
        ...
        type: "provoking",
        icon: "👑",
        prompt: "Абаддон, величественный император Ндравна. Мужчина высокого роста в темных доспехах, не сковывающих его движения, обвитых костями. Он не носит шлем, его темные волосы средней длины уложены назад, а на лице выделяются уверенный орлиный нос и густые темные брови. Он величественен, путешествует с личным отрядом уверенных людей в темных доспехах (среди которых может быть гордая разумная нежить с черепами, но не глупые безмозглые скелеты). Он не является некромантом, а правит объединенной великой империей Ндравн, где уживаются люди со своими идеалами, не нападая друг на друга. Он рассудителен, тверд как кремень и абсолютно безжалостен к слабостям.",
        isLegendary: true
      };
    }
  }
...
${loreGuidelines}`;
      ...
      prompt = `Ты — Летописец Ничейных Земель во вселенной Абаддона. Опиши короткую, суровую и грязную летопись-эпитафию в стиле Джо Аберкромби (темное фэнтези, реализм, цинизм, грязь, холодный дождь). Действие разворачивается на Ничейных Землях — суровом фронтире, где беспрерывно бьются Империя Света, разрозненные людские королевства, дикий Каргахаул, Деревянные Люди, Хаос и Нежить. Изредка тут пробегает разведка эльфов, бродячие тролли или гарпии с небесных островов. Потусторонняя Бездна — это лишь скрытый запредельный мир в нашем мире, за гранью человеческого восприятия.
...
${loreGuidelines}`;
      ...
      prompt = `Ты — Летописец Ничейных Земель во вселенной Абаддона. Опиши короткую, суровую летопись в стиле Джо Аберкромби (запах сырой земли, хруст костей, темнота, горькая ирония). Действие разворачивается на Ничейных Землях — суровом фронтире, где сталкиваются Империя Света, разрозненные людские королевства, дикий Каргахаул, Деревянные Люди, Хаос и Нежить. Иногда здесь можно встретить разведку эльфов, еще реже бродячих троллей или гарпий с небесных островов. Потусторонняя Бездна — это скрытый потусторонний мир в нашем мире, нечто за пределами восприятия.
...
const handleWinActiveSession = (task) => {
  ...
  const handleRequestReviewQuestions = async () => {
    ...
    playClick();
    try {
      const prompt = `Ты — Оракул Ничейных Земель во вселенной Абаддона (grim-dark RPG фэнтези дух). Твоя цель — расспросить Изгнанника о деталях по планируемому контракту: «${currentCard.title}» на Ничейных Землях (где ведут войну Империя Света, Каргахаул, Деревянные Люди, Хаос и Нежить). Шаги прорыва: ${currentCard.steps ? currentCard.steps.join(', ') : 'нет'}.
...
Перепиши название задачи и её шаги на основе этой инструкции. Выведи ответ строго в формате JSON:
{
  ...
  const handleDetailedDeconstruction = async () => {
    ...
    playClick();
    try {
      const prompt = `Ты — Оракул Ничейных Земель во вселенной Абаддона. Разложи задачу «${currentCard.title}» на 5-8 максимально мелких, понятных физических микро-шагов в суровых условиях Ничейных Земель, чтобы человек с СДВГ мог приступить к ней без паники и ступора. Текущие шаги: ${currentCard.steps ? currentCard.steps.join(', ') : 'нет'}. Выведи ответ строго в формате JSON: { "steps": ["микро-шаг 1", "микро-шаг 2", ...] }`;
