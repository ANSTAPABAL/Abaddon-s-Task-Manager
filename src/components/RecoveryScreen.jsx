...
export default function RecoveryScreen({ 
  ...
  const handleStartHelpChat = async () => {
    ...
    const initialPrompt = `Ты — Оракул Ничейных Земель во вселенной Абаддона (Oracle of the No Man's Lands), древний, таинственный и суровый наставник. Ты транслируешь волю через шепот запредельной Бездны (которая представляет собой потусторонний мир в нашем мире, нечто за пределами человеческого восприятия). К тебе пришел изгнанник по имени ${character.name} (класс: ${character.class}, уровень: ${character.level}, здоровье разума: ${character.hp}/100) в поисках направления. Он растерян, парализован или перегружен задачами.
...
5. Говори ИСКЛЮЧИТЕЛЬНО на языке темного фэнтези и вселенной Абаддона. Не используй термины реального мира или психологии (никаких слов: СДВГ, дофамин, прокрастинация, тайм-менеджмент, синдром, терапия и т.д.). Вместо "задачи" говори "контракты" или "обеты", вместо "усталость" говори "слабость духа" или "оковы плоти". Действие разворачивается в Ничейных Землях — суровом холодном фронтире Абаддона, где беспрерывно воюют Империя Света (люди), разрозненные людские королевства, дикий Каргахаул, Деревянные Люди, Хаос и Нежить. Иногда здесь можно встретить разведку эльфов, еще реже бродячих троллей или опасных гарпий с небесных островов. Потусторонняя Бездна — это потусторонний мир в нашем мире, запредельный мир, нечто за пределами восприятия, пробивающийся сквозь ткань реальности.`;
    ...
    try {
      ...
      if (!response.ok) throw new Error("Оракул безмолвствует");
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const fallbackText = `«Шум битвы в Ничейных Землях и гул Бездны глушат наши голоса. Но помни, изгнанник: твоя воля — твой единственный щит. Выбери один контракт, сделай первый шаг, и мрак отступит.»`;
              ...
              return (
                ...
                >
                  <div style={{ fontSize: '0.75rem', color: isOracle ? 'var(--color-mana-glow)' : 'var(--color-relic-glow)', fontWeight: 'bold', marginBottom: '4px', fontFamily: 'var(--font-rpg)', fontStyle: 'normal' }}>
                    {isOracle ? '🔮 ОРАКУЛ НИЧЕЙНЫХ ЗЕМЕЛЬ' : '🕯️ ИСПОВЕДЬ ИЗГНАННИКА'}
                  </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, padding: '2rem 1rem', textAlign: 'center' }}>
          ...
          <div style={{ maxWidth: '600px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem' }}>
            ...
            <p style={{ fontSize: '0.95rem', lineHeight: '1.6', color: 'var(--color-bone)' }}>
              Когда когнитивный шум заглушает голос разума, и вы не знаете, к какому обету подступиться — воззовите к Оракулу Ничейных Земель. 
              <br />
