...
export default function App() {
  ...
  const parseMessyTasks = async (textBlob) => {
    const systemPrompt = `Ты — Бездна во вселенной Абаддона. Твоя задача — взять хаотичные мысли СДВГ-пользователя и превратить их в структурированный JSON-массив задач.
Группируй дела по сущностям:
- "hunt" (охота) — мелкие задачи до 25 минут, дающие импульс старта.
- "siege" (осада) — большие комплексные проекты свыше 40 минут.
- "relic" (реликвия) — редкие, но важные дела.
- "corpse" (труп прошлого) — давно заброшенные, висящие дела.

Оцени "toxicity" (токсичность задачи): "scary" (страшная), "vague" (мутная), "tedious" (скучная), "standard" (обычная).
Обязательно добавь 3-5 простых микро-шагов в массив "steps".
