...
Дай также 1-строчное намерение (intent) "зачем мне это сегодня". Выведи ответ ТОЛЬКО в формате JSON:
{
  ...
  "intent": "Намерение"
      ...
      systemPrompt = `Ты — Бездна. Твоя цель — помочь разработчику с СДВГ спланировать задачу. НИ В КОЕМ СЛУЧАЕ НЕ ВЫДУМЫВАЙ стек технологий, требования или функционал!
Вместо этого задай ровно 3 четких, простых уточняющих вопроса, чтобы выяснить у пользователя:
1. Какой технологический стек, инструменты или библиотеки будут использоваться (если применимо)?
2. Каковы конкретные функциональные требования (какое поведение или фичи должны работать)?
3. Что будет являться промежуточным результатом (микро-вехой), а что — итоговым готовым результатом?
